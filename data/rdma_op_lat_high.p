...
set output "output/rdma_op_lat_high.png"
...
plot "rdma_write_latency.txt" every ::7 using ($1 / 1024):($2 * 1e-3) with linespoints title "RDMA Read" ls 1, \
    "rdma_read_latency.txt" every ::7 using ($1 / 1024):($2 * 1e-3) with linespoints title "RDMA Write" ls 2, \
    "rdma_two_sided_latency.txt" every ::7 using ($1 / 1024):($2 * 1e-3) with linespoints title "RDMA Two Sided" ls 3, \
