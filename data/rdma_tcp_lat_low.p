...
set output "output/rdma_tcp_lat_low.png"
...
plot "rdma_write_latency.txt" every ::::8 using ($1):($2 * 1e-3) with linespoints title "RDMA Read" ls 1, \
    "tcp_eth_latency.txt" every ::::8 using ($1):($2 * 1e-3) with linespoints title "TCP over Ethernet" ls 2, \
    "tcp_latency.txt" every ::::8 using ($1):($2 * 1e-3) with linespoints title "TCP over Infiniband" ls 3
